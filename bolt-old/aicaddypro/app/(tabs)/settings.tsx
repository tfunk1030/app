...
import { getNearestStationsFromDevice, NearbyStation } from '@/src/services/weather/stations';
...
// Nearby stations dropdown component
const NearbyStationsPicker = memo(
  ({ updateSettings }: { updateSettings: (settings: any) => void }) => {
    ...
    const load = async () => {
      try {
        setLoading(true);
        const list = await getNearestStationsFromDevice(10);
        setStations(list);
        setOpen(true);
      } finally {
        setLoading(false);
        ...
        {!open ? (
          <Pressable onPress={load} accessibilityRole="button">
            <Text style={styles.actionText}>
              {loading ? 'Finding nearby…' : 'Find Nearby Stations'}
            </Text>
          </Pressable>
        ) : (
          <View style={styles.dropdown}>
            {stations.length === 0 ? (
              <Text style={styles.dropdownItemText}>No stations found</Text>
            ) : (
              stations.map((s, i) => (
                <Pressable
                  key={`${s.icao || 'station'}-${i}`}
                  onPress={async () => {
                    await updateSettings({ metarIcao: s.icao });
                    setOpen(false);
                  }}
                  style={styles.dropdownItem}
                >
                  <Text style={styles.dropdownItemText}>
                    {typeof s.icao === 'string' ? s.icao : ''}
                    {s.name ? ` · ${s.name}` : ''}
...
NearbyStationsPicker.displayName = 'NearbyStationsPicker';
  ...
  ({ settings, updateSettings }: { settings: any; updateSettings: (settings: any) => void }) => {
    ...
    return (
      <GlassCard style={{ ...styles.section, backgroundColor: palette.colors.surfaceAlt }}>
        ...
        {/* Forecast Provider */}
        <View style={styles.unitGroup}>
          <Text style={styles.unitLabel}>Forecast Provider</Text>
          <View style={styles.buttonGroup}>
            <Button
              variant={settings.forecastProvider === 'auto' ? 'default' : 'secondary'}
            ...
            <Button
              variant={settings.forecastProvider === 'stormglass' ? 'default' : 'secondary'}
              onPress={() =>
            ...
            <Button
              variant={settings.forecastProvider === 'openmeteo' ? 'default' : 'secondary'}
              onPress={() =>
                updateSettings({ forecastProvider: 'openmeteo', forceOpenMeteo: true })
              }
              style={{ flex: 1, minWidth: 0 }}
            >
              Open‑Meteo
          ...
          <View style={{ marginTop: 8, gap: 6 }}>
            ...
            >
              Stormglass: NOAA‑based modeled forecasts with steady wind/gust relationships.
              Typically more consistent near coasts and large water bodies; great if you value
              stability round‑to‑round.
            </Text>
            <Text
              style={{
                color: palette.colors.textMuted,
            ...
            >
              Open‑Meteo: Multi‑model global forecasts with quick coverage everywhere. Good for
              travel and broad availability; can vary more by region versus NOAA.
            </Text>
            <Text
              style={{
                color: palette.colors.textMuted,
            ...
            >
              Tip: Different sources may yield slightly different wind values due to model inputs
              and update cadence. A small shift after switching is normal.
            </Text>
          </View>
        </View>

        {/* Debug / Provider Flags */}
        <View style={styles.unitGroup}>
          <View style={styles.clubHeader}>
            <Text
              style={[styles.unitLabel, { flex: 1 }]}
              numberOfLines={1}
              adjustsFontSizeToFit
              minimumFontScale={0.75}
            >
              Force Open‑Meteo (debug)
            </Text>
            <Switch
              value={!!settings.forceOpenMeteo}
              onValueChange={value => updateSettings({ forceOpenMeteo: value })}
              trackColor={{ false: palette.colors.border, true: palette.colors.brand }}
              thumbColor={
                settings.forceOpenMeteo ? palette.colors.textPrimary : palette.colors.textMuted
              }
              accessibilityLabel="Force Open‑Meteo provider"
            />
          </View>
          <View style={[styles.clubHeader, { marginTop: 8 }]}>
            <Text
              style={[styles.unitLabel, { flex: 1 }]}
              numberOfLines={1}
              adjustsFontSizeToFit
            ...
            <Switch
              value={!!settings.useMetar}
              onValueChange={value => updateSettings({ useMetar: value })}
              trackColor={{ false: palette.colors.border, true: palette.colors.brand }}
              thumbColor={settings.useMetar ? palette.colors.textPrimary : palette.colors.textMuted}
              accessibilityLabel="Enable METAR blending"
            />
          </View>
          <View style={{ marginTop: 8 }}>
            <Text style={styles.unitLabel}>METAR ICAO</Text>
            <TextInput
              placeholder="e.g., KJAX"
              value={settings.metarIcao ?? ''}
              onChangeText={text => updateSettings({ metarIcao: text.trim().toUpperCase() })}
              style={[styles.input, styles.editInput]}
              placeholderTextColor={palette.colors.textMuted}
              autoCapitalize="characters"
              accessibilityLabel="METAR ICAO code"
            />
            {/* Nearby stations dropdown */}
            <NearbyStationsPicker updateSettings={updateSettings} />
            <View style={{ marginTop: 8, gap: 6 }}>
              <Text
                style={{
                  color: palette.colors.textMuted,
              ...
              >
                METAR (airport observations): Real measurements of wind, temperature, pressure, and
                humidity reported every 5–60 minutes. Most accurate near the station; not a
                forecast.
              </Text>
              <Text
                style={{
              ...
              >
                Use when you’re playing close to an airport and want ground‑truth winds. We blend
                METAR with the forecast (strongest influence within ~1 hour of observation).
              </Text>
              <Text
                style={{
                  color: palette.colors.textMuted,
              ...
              >
                How to use: Toggle “Use METAR”, set an ICAO (e.g., KJAX), or tap “Find Nearby
                Stations” to pick from a list. If a station is stale or invalid, the app falls back
                automatically.
              </Text>
            </View>
          </View>
