import { StormglassResponse } from '../enhanced-environmental-service';
...
export function validateWeatherAPIResponse(response: StormglassResponse | WeatherbitResponse) {
  if (!response) {
    throw new ValidationError('Invalid weather API response format');
  }

  // Handle Stormglass response
  if ('hours' in response) {
    const weatherData = response.hours[0];
    ...
    numericFields.forEach(field => {
      ...
      const value =
        weatherData[parent as keyof typeof weatherData]?.[
          child as keyof (typeof weatherData)[keyof typeof weatherData]
        ];
    ...
    weatherData.gust.noaa = Math.max(weatherData.gust.noaa, windSpeed);

    // Validate value ranges
    if (weatherData.airTemperature.noaa < -100 || weatherData.airTemperature.noaa > 150) {
      throw new ValidationError('Temperature out of valid range');
    }
    if (weatherData.humidity.noaa < 0 || weatherData.humidity.noaa > 100) {
      throw new ValidationError('Relative humidity out of valid range');
    }
    if (weatherData.pressure.noaa < 800 || weatherData.pressure.noaa > 1200) {
      throw new ValidationError('Pressure out of valid range');
    }
    if (weatherData.windSpeed.noaa < 0 || weatherData.windSpeed.noaa > 200) {
      throw new ValidationError('Wind speed out of valid range');
    }
    if (weatherData.windDirection.noaa < 0 || weatherData.windDirection.noaa > 360) {
      throw new ValidationError('Wind direction out of valid range');
    }
    if (weatherData.gust.noaa < 0 || weatherData.gust.noaa > 300) {
