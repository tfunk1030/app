  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const preference = (settings.forecastProvider ||
          (settings.forceOpenMeteo ? 'openmeteo' : 'auto')) as 'auto' | 'stormglass' | 'openmeteo';
        const data = await fetchHourlyWind(lat, lon, 5, preference as any);
        if (!cancelled) {
          setPoints(data);
          setLoading(false);
        }
