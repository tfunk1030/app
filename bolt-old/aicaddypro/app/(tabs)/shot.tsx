...
export default function ShotCalculatorScreen() {
  ...
  const [lie, setLie] = useState<'fairway' | 'rough' | 'downhill' | 'uphill'>('fairway');
  const [elevationDelta, setElevationDelta] = useState(0); // in yards (store canonical unit)
  ...
  return (
    <SafeAreaView style={{ flex: 1 }} edges={['top', 'left', 'right']}>
      ...
      >
        ...
        {/* Quick Adjust: Lie & Elevation (display-only hints) */}
        <GlassCard style={{ marginBottom: 16 }}>
          ...
          >
            <Button
              variant={lie === 'fairway' ? 'default' : 'secondary'}
              size="sm"
              onPress={() => setLie('fairway')}
              style={{ flex: 1, minWidth: 0 }}
            >
              Fairway
            </Button>
            <Button
              variant={lie === 'rough' ? 'default' : 'secondary'}
              size="sm"
              onPress={() => setLie('rough')}
              style={{ flex: 1, minWidth: 0 }}
            >
              Rough
            </Button>
            <Button
              variant={lie === 'downhill' ? 'default' : 'secondary'}
              size="sm"
              onPress={() => setLie('downhill')}
              style={{ flex: 1, minWidth: 0 }}
            >
              Downhill
            </Button>
            <Button
              variant={lie === 'uphill' ? 'default' : 'secondary'}
              size="sm"
              onPress={() => setLie('uphill')}
              style={{ flex: 1, minWidth: 0 }}
            >
              Uphill
            </Button>
          ...
          {/* Elevation to Target slider (store in yards, display in current unit) */}
          {(() => {
            const displayValue =
              settings.distanceUnit === 'yards'
                ? elevationDelta
                : Math.round(convertDistance(elevationDelta, 'meters'));
            const min = settings.distanceUnit === 'yards' ? -30 : -27; // approx -25m to +25m
            const max = settings.distanceUnit === 'yards' ? 30 : 27;
            const unit = settings.distanceUnit === 'yards' ? ' yds' : ' m';
            return (
              <View style={{ marginTop: 4 }}>
                <Slider
                  ...
                  onValueChange={v =>
                    setElevationDelta(
                      settings.distanceUnit === 'yards' ? v : Math.round(convertDistance(v, 'yards'))
                    )
                  }
                  min={min}
                  max={max}
                  step={1}
                  label="Elevation to Target"
                  unit={unit}
                />
                <Text
                  style={{ color: palette.colors.textMuted, fontSize: scaledFontSize(12), marginTop: 6 }}
                  accessibilityRole="text"
                >
                  Tip: Quick Adjust hints are for awareness and are not yet applied to the recommendation.
                </Text>
              </View>
            );
          })()}
